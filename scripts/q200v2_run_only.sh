#!/usr/bin/env bash
# q200v2_run_only.sh — final stage with the ALREADY-RUNNING labeled serve.
# Guards + admission config + certified runner. No serve relaunch.
set -euo pipefail
...
EPOCH=$epoch
log "run-only stage: epoch=$EPOCH"

curl -sf -m 10 http://127.0.0.1:30000/v1/models | grep -q model || { log "FATAL serve not ready"; exit 3; }
log "serve ready"
...
# verify labels actually landed on the containers (guard requires them)
docker inspect dsv41-rank --format '{{index .Config.Labels "org.r0b0tlab.epoch"}}' | grep -q "$EPOCH" \
  || { log "FATAL rank0 labels missing/wrong epoch"; docker inspect dsv41-rank --format '{{.Config.Labels}}'; exit 4; }
ssh "${SSH_OPTS[@]}" r0b0tdgx@$N2 "docker inspect dsv41-rank --format '{{index .Config.Labels \"org.r0b0tlab.epoch\"}}'" | grep -q "$EPOCH" \
  || { log "FATAL rank1 labels missing/wrong epoch"; exit 4; }
log "labels verified on rank0+rank1"
...
nohup python3 "$RUNNER/scripts/guard_unified_memory.py" \
  ...
  --peer-profile-sha256 "$PROFILE_SHA" \
...
# admission config must be readable at the same path ON N2 (guards read their local copy)
scp -q -i $HOME/.ssh/id_ed25519_crs812 -o IdentitiesOnly=yes -o BatchMode=yes \
    -o UserKnownHostsFile=$HOME/.ssh/known_hosts_crs812_fabric \
    "$EV/admission-config.json" "r0b0tdgx@$N2:$EV/admission-config.json"
ssh "${SSH_OPTS[@]}" r0b0tdgx@$N2 "mkdir -p '$GUARD_DIR/rank1'; rm -f '$GUARD_DIR/rank1/request-state.json'; nohup python3 /tmp/guard_unified_memory.py \
  --container dsv41-rank --candidate-id '$CANDIDATE' --candidate-source-sha '$SOURCE_SHA' \
...
sleep 60
log "guards started"
tail -2 "$GUARD_DIR/rank0/guard.log" 2>/dev/null || true
